...
ALTER TABLE tokens
    ADD CONSTRAINT unique_token UNIQUE (user_id);

SELECT * FROM match
inner join match_to_teams
    on match.id = match_to_teams.match_id;

INSERT INTO match (datetime, result, sport_id, tournament_id, name)
values ('2018-06-11', 'hello', 1, 1, 'final');

INSERT INTO match_to_teams (team_1_id, team_2_id, match_id)
values (1, 2, 1), (2, 1, 1);

INSERT INTO match_to_teams (team_1_id, team_2_id, match_id) VALUES (1,2,2);



SELECT * FROM comment INNER JOIN "user" ON "user".id = comment.author_id WHERE post_id = ?

DELETE FROM comment WHERE post_id = 1;
SELECT * FROM match INNER JOIN match_to_sportsman ON match.id = match_to_sportsman.match_id WHERE tournament_id = ?

UPDATE match SET name = 'semi-final' WHERE id = 1;
